...
class NewSessionUseCase:
    ...
    async def prompt_user(self, recepient_id: str):
        ...
        if customer is None:
            session_type = SessionType(id=1, name="NEW_SESSION")
            session = UserSession(
                id=recepient_id, user=None, session_type=session_type, current_step=1
            )
            print("saving new session...")
            await self.session_service.save_customer_session(customer_session=session)
            print("prompting user on whatsapp")
        ...
        session = UserSession(
            id=recepient_id, user=customer, session_type=session_type, current_step=1
        )
